...
const ActionButton = ({ icon, label, isActive, onClick, isLoading }) => (
  <button
    onClick={onClick}
    disabled={isLoading}
...
// Landing stage strings (no TypeScript types here)
...
const useTypingEffect = (lines, typingSpeed = 25, onComplete) => {
...
const LandingPage = ({ onLogin }) => {
  ...
  // Mock user database
  const userDB = useRef({ user: 'password' });
...
// App stage strings: 'landing' | 'transition' | 'main'
...
const App = () => {
  ...
  const [activeAction, setActiveAction] = useState(ActionType.Explain);
  ...
  const handleAction = useCallback(async (action) => {
    ...
    try {
      ...
      setError(err && err.message ? err.message : String(err) || "An unexpected error occurred.");
    } finally {
      setIsLoading(false);
    }
  }, [code]);
  ...
  setCode(typeof text === 'string' ? text : String(text));
      };
